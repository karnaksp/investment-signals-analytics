# Аналитика инвестиционных сигналов: от события к решению

## Задача

`investment-signals` генерирует рыночные сигналы, но для эксплуатации важно не только увидеть факт сигнала.
Нужно понимать:
...
Этот репозиторий закрывает аналитический контур вокруг этих вопросов: берет потоковые данные из Postgres, пересчитывает dbt-витрины,
...
- можно безопасно менять правила оценки и витрины без риска сломать прием сигналов;
- можно запускать пересчет вручную или по расписанию;
- можно версионировать dbt-модели и панель Lightdash как код;
- можно отделить рабочий API от аналитических таблиц и BI-интерфейса.
...
Нормализует события из рабочей таблицы:
...
- ослабить или уточнить паузу между уведомлениями для сильных `volume_spike`;
...
- Lightdash открывает панель `Операционный обзор сигналов`;
- панель показывает ключевые показатели, распределение решений, узкие места доставки и детальные таблицы.
